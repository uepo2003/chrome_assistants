...
export const SYSTEM_PROMPT = [
  ...
  '',
  'Decide the SINGLE next action and reply with ONE JSON object only. Output RAW',
  'JSON — no prose, no explanation, no markdown code fences (no ``` wrapping).',
  '',
...
export const PLAN_SYSTEM_PROMPT = [
  ...
  '',
  'Reply with ONE JSON ARRAY only. Output RAW JSON — no prose, no markdown code',
  'fences (no ``` wrapping). Maximum 8 steps. Minimum 1 step.',
  '',
...
export const STEP_SYSTEM_PROMPT = [
  ...
  'object only — output RAW JSON, no prose outside the JSON, no markdown code',
  'fences (no ``` wrapping).',
  '',
